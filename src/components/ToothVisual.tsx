...
export default function ToothVisual({
  ...
  const gradientId = `tooth-grad-${filterId.replace(/:/g, '')}`;

  return (
    <div 
      className={`relative select-none ${className}`}
      style={{
        filter: 'drop-shadow(-3px -3px 5px rgba(14,165,233,0.9)) drop-shadow(3px 3px 5px rgba(239,68,68,0.9)) drop-shadow(0 0 12px rgba(14,165,233,0.4))',
    ...
    >
      ...
      {/* Accessories */}
      {accessory === 'stars' && (
        <div className="absolute top-[20%] right-[10%] text-ruby-500 text-lg animate-bounce">
      ...
      {accessory === 'crown' && (
        <div className="absolute top-[-3%] left-1/2 -translate-x-1/2 text-ruby-500 drop-shadow-sm">
          <Star className="w-7 h-7 fill-ruby-500" />
        </div>
      )}

      {accessory === 'pin' && (
        <div className="absolute top-[18%] left-[12%] text-clinic-600 animate-pulse">
          <MapPin className="w-5 h-5 fill-clinic-100" />
        </div>
      )}
    </div>
  );
